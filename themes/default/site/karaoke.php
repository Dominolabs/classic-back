...
/* @var $karaokeBannerImages array */
/* @var $imageWidth int */
/* @var $imageHeight int */
/* @var $karaokePageMenuDescr string */
/* @var $eventCategoryId int */
/* @var $albumCategoryId int */
/** @var $karaokePageSocialNetworkCategoryId int */
/* @var $pageUrl string */
...
$karaokePageMenuPdf = isset(Yii::$app->params['karaokePageMenuPdf']) ? Yii::$app->params['karaokePageMenuPdf'] : null;
?>
<div class="page-content__content  section-page">
    <section class="section-page__first-block  page-content__first-block  first-block" style="background-image: url('<?= $this->params['backgroundImage'] ?>')">
        <div class="first-block__logo  sections-logo">
            <div class="sections-logo__logo">
                ...
                <?php if ($karaokePageSocialNetworkCategoryId > 0): ?>
                    <?= SocialNetworks::widget(['socialNetworkCategoryId' => $karaokePageSocialNetworkCategoryId]) ?>
                <?php endif; ?>
            </div>
        </div>
    </section>
    <section class="section-page__interior  interior">
        ...
        <?php if (!empty($karaokeBannerImages)): ?>
        <div class="interior__carousel-wrap">
            <div class="interior__carousel  owl-carousel">
                <?php foreach ($karaokeBannerImages as $karaokeBannerImage): ?>
                <div class="interior__carousel-item">
                    <div class="interior__carousel-img-wrap">
                        <img src="<?= BannerImage::getImageUrl($karaokeBannerImage['image'], 931, 650) ?>"
                             alt="<?= $karaokeBannerImage['title'] ?>"
    ...
    <section class="section-page__menu-block  menu-block">
        ...
        <?php if (!empty(Yii::$app->params['karaokePageMenuImage'])): ?>
            <div class="menu-block__img-wrap">
                <img class="menu-block__img" src="<?= SettingForm::getImageUrl(Yii::$app->params['karaokePageMenuImage'], 713, 563) ?>" />
            </div>
        <?php endif; ?>
        <div class="menu-block__descr-wrap">
            <p class="menu-block__text"><?= $karaokePageMenuDescr ?></p>
            <div class="menu-block__btns-wrap">
                <a class="menu-block__btn  btn  g-mob-hide" href="<?= Url::to(['menu']) ?>">
                    <?= Yii::t('family', 'Перейти в меню') ?>
                </a>
                <?php if (!empty($karaokePageMenuPdf)): ?>
                    <a class="menu-block__btn  btn  g-mob-hide" href="<?= Url::to('/'. $karaokePageMenuPdf) ?>" target="_blank">
        ...
        <div class="menu-block__btns-wrap">
            ...
            <?php if (!empty($karaokePageMenuPdf)): ?>
                <a class="menu-block__btn  btn  g-large-hide  g-desk-hide  g-tablet-hide" href="<?= Url::to('/'. $karaokePageMenuPdf) ?>" target="_blank">
                    <?= Yii::t('family', 'Переглянути меню бару') ?>
                </a>
            <?php endif; ?>
        </div>
